          ...
          nodes: {
            ...
            items: {
              ...
              properties: {
                ...
                // Location specific properties (when entityType is Location)
                locationType: { type: "string", description: "Type of location (City, Country, Region, Building, Virtual, etc.)" },
                coordinates: { 
                  type: "object", 
                  properties: {
                  ...
                  description: "Geographical coordinates for Location nodes"
                },
                locationSignificance: { type: "string", description: "Historical, cultural, or personal importance of this location" },
                
                // Location relationships (used to create relationships, not stored as attributes)
                containedWithin: { type: "string", description: "Creates a CONTAINED_IN relationship to another Location" },
                eventsOccurred: { type: "array", items: { type: "string" }, description: "Creates OCCURRED_AT relationships from Events to this Location" },
                
                // Person specific properties (when subType is Person)
                aliases: { type: "array", items: { type: "string" }, description: "Alternative names or nicknames for Person" },
                personalityTraits: { 
                  type: "array", 
                  items: { 
                    type: "object", 
                    properties: {
                      trait: { type: "string", description: "Trait name (e.g., Analytical, Pragmatic)" },
                      evidence: { type: "array", items: { type: "string" }, description: "Evidence supporting this trait" },
                      confidence: { type: "number", description: "Confidence in this trait attribution (0.0-1.0)" }
                    }
                  }, 
                  description: "Personality traits for Person nodes" 
                },
                cognitiveStyle: { 
                  type: "object", 
                  properties: {
                    decisionMaking: { type: "string", description: "How the person makes decisions (e.g., Methodical, Intuitive)" },
                    problemSolving: { type: "string", description: "Approach to problem-solving" },
                    worldview: { type: "string", description: "Person's fundamental perspective" },
                    biases: { type: "array", items: { type: "string" }, description: "Observed cognitive biases" }
                  },
                  description: "Cognitive style for Person nodes" 
                },
                emotionalDisposition: { type: "string", description: "Overall emotional tendency (e.g., Reserved, Volatile)" },
                emotionalTriggers: { 
                  type: "array", 
                  items: { 
                    type: "object", 
                    properties: {
                      trigger: { type: "string", description: "Events/situations causing strong emotional responses" },
                      reaction: { type: "string", description: "Typical emotional response" },
                      evidence: { type: "array", items: { type: "string" }, description: "Textual evidence" }
                    }
                  }, 
                  description: "Emotional triggers for Person nodes" 
                },
                interpersonalStyle: { type: "string", description: "How the person typically interacts with others" },
                powerDynamics: { 
                  type: "object", 
                  properties: {
                    authorityResponse: { type: "string", description: "How they respond to authority" },
                    subordinateManagement: { type: "string", description: "How they manage subordinates" },
                    negotiationTactics: { type: "array", items: { type: "string" }, description: "Observed negotiation approaches" }
                  },
                  description: "Power dynamics for Person nodes" 
                },
                loyalties: { 
                  type: "array", 
                  items: { 
                    type: "object", 
                    properties: {
                      target: { type: "string", description: "Person, institution, or concept" },
                      strength: { type: "number", description: "Intensity of loyalty (0.0-1.0)" },
                      evidence: { type: "array", items: { type: "string" }, description: "Supporting textual evidence" }
                    }
                  }, 
                  description: "Loyalties for Person nodes" 
                },
                coreValues: { 
                  type: "array", 
                  items: { 
                    type: "object", 
                    properties: {
                      value: { type: "string", description: "Value (e.g., National security, Global stability)" },
                      importance: { type: "number", description: "Relative importance (0.0-1.0)" },
                      consistency: { type: "number", description: "How consistently upheld (0.0-1.0)" }
                    }
                  }, 
                  description: "Core values for Person nodes" 
                },
                ethicalFramework: { type: "string", description: "Ethical approach (e.g., Realist, Utilitarian)" },
                psychologicalDevelopment: { 
                  ...
                  items: { 
                    ...
                    properties: {
                      ...
                      changes: { type: "string", description: "Notable psychological shifts" },
                      catalysts: { type: "array", items: { type: "string" }, description: "Events triggering changes" }
                    }
                  }, 
                  description: "Psychological development over time for Person nodes" 
                },
                narrativeTreatment: { 
                  type: "object", 
                  properties: {
                    authorBias: { type: "number", description: "Detected authorial bias (-1.0 to 1.0)" },
                    portrayalConsistency: { type: "number", description: "Consistency across sources (0.0-1.0)" },
                    controversialAspects: { type: "array", items: { type: "string" }, description: "Disputed psychological features" }
                  },
                  description: "Narrative treatment for Person nodes" 
                },
                modelConfidence: { type: "number", description: "Overall confidence in Person profile (0.0-1.0)" },
                personEvidenceStrength: { type: "number", description: "Strength of supporting evidence for Person (0.0-1.0)" },
